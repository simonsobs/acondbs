...
@pytest.fixture()
def mock_backup_db_as_csv_to_github_(monkeypatch: pytest.MonkeyPatch) -> mock.Mock:
    from acondbs.db import backup

    y = mock.Mock()
    monkeypatch.setattr(backup, 'backup_db_as_csv_to_github_', y)
    return y
...
def test_backup_db_as_csv_to_github(
    app: Flask, mock_lock_path: Path, mock_backup_db_as_csv_to_github_: mock.Mock
) -> None:
    del mock_lock_path
