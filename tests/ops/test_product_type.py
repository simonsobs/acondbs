...
def test_create(app: Flask, field_ids: Optional[list[int]]) -> None:
    with app.app_context():
...
@pytest.mark.parametrize('field_ids', params)
def test_create_error(app: Flask, field_ids: list[int]) -> None:
    with app.app_context():
        count = ProductType.query.count()
...
def test_update(
    app: Flask, field_ids: Optional[list[int]], expected_field_ids: list[int]
) -> None:
    type_id = 2
...
@pytest.mark.parametrize('field_ids', params)
def test_update_error(app: Flask, field_ids: list[int]) -> None:
    type_id = 2
...
def test_delete(app: Flask) -> None:
    ...
    with app.app_context():
        count = ProductType.query.count()
        ops.delete_product_type(type_id=type_id)
        ops.commit()
