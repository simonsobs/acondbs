...
def test_query(app):
    ...
    with app.app_context():
        ...
        token1, token2 = user1.admin_app_tokens
...
def test_add(app):
    ...
    with app.app_context():
        user1 = GitHubUser.query.filter_by(login="octocat").one()
        ntokens = len(user1.admin_app_tokens) # = 2
        token3 = GitHubAdminAppToken(token="token_003", scope="read:org", user=user1)
        sa.session.commit()

    with app.app_context():
        user1 = GitHubUser.query.filter_by(login="octocat").one()
        assert ntokens + 1 == len(user1.admin_app_tokens)
        assert 'token_003' in [t.token for t in user1.admin_app_tokens]
...
def test_delete(app):
    ...
    with app.app_context():
        user1 = GitHubUser.query.filter_by(login="octocat").one()
        ntokens = len(user1.admin_app_tokens) # = 2
        token1 = user1.admin_app_tokens[0]
        sa.session.delete(token1)
        sa.session.commit()

    with app.app_context():
        user1 = GitHubUser.query.filter_by(login="octocat").one()
        assert ntokens - 1 == len(user1.admin_app_tokens) # 1

    with app.app_context():
        user1 = GitHubUser.query.filter_by(login="octocat").one()
        token2 = user1.admin_app_tokens[0]
        sa.session.delete(token2)
        sa.session.commit()

    with app.app_context():
        user1 = GitHubUser.query.filter_by(login="octocat").one()
        assert ntokens - 2 == len(user1.admin_app_tokens) # 0
