"""copy to attribute tables

Revision ID: 2910eaefa574
Revises: 41e5ff6f2878
Create Date: 2021-09-14 15:04:33.300075
...
revision = "2910eaefa574"
down_revision = "41e5ff6f2878"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    for p in products:
        for c in columns_text:
            AttributeText(name=c, product=p, value=getattr(p, c))
        for c in columns_date:
            AttributeDate(name=c, product=p, value=getattr(p, c))
        for c in columns_date_time:
            AttributeDateTime(name=c, product=p, value=getattr(p, c))
    session.commit()
